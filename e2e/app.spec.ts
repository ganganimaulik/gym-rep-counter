...
test.describe('Gym Rep Counter E2E Tests', () => {
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    try {
      await request.delete('http://127.0.0.1:8080/emulator/v1/projects/gym-rep-counter/databases/(default)/documents');
    } catch (e) {
    ...
    try {
      await request.delete('http://127.0.0.1:9099/emulator/v1/projects/gym-rep-counter/accounts');
    } catch (e) {
  ...
  test('1. Bottom Tab Bar Navigation', async ({ page }) => {
    ...
    await expect(page.getByText('No workout history yet. Complete a set to get started!')).toBeVisible();
  ...
  test('2. Workout Screen - Routine selection, skipping sets validation, and timers', async ({ page }) => {
    // 1. Pick a workout
  ...
  test('4. Routines Screen - Create, Edit, and Delete Routines', async ({ page }) => {
    await page.getByText('Routines', { exact: true }).click();

    // Create routine
    await page.getByPlaceholder('Routine name (e.g. Chest & Triceps)').fill('Chest Day');
    ...
    const chestDayCard = page.locator('div.rounded-2xl', { hasText: 'Chest Day' }).last();
    ...
    await page.locator('[data-testid="edit-exercise-name"]').fill('Incline Bench Press');
  ...
  test('6. Analytics Screen - Health & TDEE setup and daily stats logging', async ({ page }) => {
    await page.getByText('Analytics', { exact: true }).click();
  ...
  test('7. Journal Screen - Diary and supplements tracking', async ({ page }) => {
    await page.getByText('Journal', { exact: true }).click();
    ...
    // Enter note body
    await page.getByPlaceholder('How did you feel today?').fill('Smashed heavy squats!');
    ...
    await page.locator('div:has-text("Popular Supplements")').locator('text=Creatine').first().click();
    await page.locator('[data-testid="add-supplement-button"]').click();
  ...
  test('8. Settings Screen - Timer settings and Firebase Emulator Cloud Sync', async ({ page }) => {
    await page.getByText('Settings', { exact: true }).click();
    ...
    await page.reload();
    await page.getByText('Settings', { exact: true }).click();
    await expect(page.locator('[data-testid="setting-countdown"]')).toHaveValue('5');
  ...
  test('9. Routines Screen - Deleting Workout Routine', async ({ page }) => {
    ...
    // Create routine
    await page.getByPlaceholder('Routine name (e.g. Chest & Triceps)').fill('Leg Day');
    await page.getByText('Create', { exact: true }).click();
    ...
    const legDayCard = page.locator('div.rounded-2xl', { hasText: 'Leg Day' }).last();
  ...
  test('10. Analytics Screen - Workout Trends, PR Table, and Exercise Trends Picker', async ({ page }) => {
    ...
    const prCard = page.locator('div.rounded-2xl', { hasText: 'Personal Records' }).last();
  ...
  test('11. Analytics Screen - Health Goal Projections, Preferences, and Weekly Averages', async ({ page }) => {
    await page.getByText('Analytics', { exact: true }).click();
    await page.getByText('Health & TDEE').click();
    ...
    await expect(page.getByText('Calculated Targets & Projections')).toBeVisible();
  ...
  test('12. Journal Screen - Note editing and deletion lifecycle', async ({ page }) => {
    await page.getByText('Journal', { exact: true }).click();

    // Add note
    await page.locator('[data-testid="add-journal-note-button"]').click();
    await page.getByPlaceholder('How did you feel today?').fill('Felt strong today');
    ...
    await expect(page.getByPlaceholder('How did you feel today?')).toHaveValue('Felt strong today');

    // Modify note text and add supplement
    await page.getByPlaceholder('How did you feel today?').fill('Felt strong today - updated');
    ...
    await expect(page.getByText('Felt strong today - updated')).not.toBeVisible();
  });

  test('13. Settings Screen - Toggles, bedtime adjustment controls, and logout', async ({ page }) => {
    await page.getByText('Settings', { exact: true }).click();

    // Toggle Eccentric voice switch
    const eccentricSwitch = page.locator('[data-testid="toggle-eccentric-voice"]');
    ...
    await page.reload();
    await page.getByText('Settings', { exact: true }).click();
    await expect(page.locator('[data-testid="sleep-start-text"]')).toHaveText('10:00 PM');
    await expect(page.locator('[data-testid="sleep-end-text"]')).toHaveText('8:00 AM');
