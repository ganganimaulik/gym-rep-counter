...
import { formatDate, formatShortDate, formatWeight, formatCalories } from '../formatters';
...
export function registerNutritionTools(server: McpServer) {
  server.tool(
    ...
    {
      days_back: z.number().min(1).max(365).optional().describe('Number of days of history. Default: 30'),
    ...
    async (args, extra) => {
      ...
      const snap = await db.collection(`users/${user.uid}/weightLogs`)
        .where('date', '>=', start)
        .where('date', '<', end)
        .orderBy('date', 'desc')
        .get();
      ...
      if (logs.length === 0) {
        return { content: [{ type: 'text' as const, text: `⚖️ Weight Log (Last ${daysBack} Days)\n\nNo weight entries found.` }] };
      }
      ...
      for (const log of logs) {
        ...
        lines.push(`${dateStr}: ${formatWeight(log.weight as number, ctx.weightUnit)}`);
      }
      ...
      const sign = delta >= 0 ? '+' : '';
      lines.push('');
      lines.push(`Trend: ${formatWeight(oldest, ctx.weightUnit)} → ${formatWeight(newest, ctx.weightUnit)} (Δ ${sign}${delta.toFixed(1)} ${ctx.weightUnit})`);
  ...
  server.tool(
    ...
    async (args, extra) => {
      ...
      const snap = await db.collection(`users/${user.uid}/calorieLogs`)
        .where('date', '>=', start)
        .where('date', '<', end)
        .orderBy('date', 'desc')
        .get();
      ...
      if (logs.length === 0) {
        return { content: [{ type: 'text' as const, text: `🔥 Calorie Log (Last ${daysBack} Days)\n\nNo calorie entries found.` }] };
      }
      ...
      for (const log of logs) {
        const dateStr = formatShortDate(log.date, tz);
        lines.push(`${dateStr}: ${formatCalories(log.calories as number, ctx.energyUnit)}`);
      }
