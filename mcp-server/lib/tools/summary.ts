  ...
  server.tool(
    ...
    "Get a complete snapshot of a day's activity including workouts, weight, calories, journal entries, and supplements.",
    {
      date: z
        ...
        .optional()
        .describe(
          'Date in YYYY-MM-DD format. Defaults to today in your timezone.',
        ),
      timezone: z
        .string()
        .optional()
        .describe(
          'IANA timezone (e.g., "America/New_York", "Asia/Kolkata"). Defaults to UTC.',
        ),
    },
    async (args, extra) => {
      ...
      if (historySnap.empty) {
        ...
        for (const [, group] of Object.entries(exerciseGroups)) {
          ...
          for (const s of group.sets.sort((a, b) => a.set - b.set)) {
            lines.push(
              `    Set ${s.set}: ${s.reps} reps @ ${formatWeight(s.weight, ctx.weightUnit)}`,
            )
          }
        }
        lines.push(
          `  Total Volume: ${formatVolume(totalVolume)} ${ctx.weightUnit}`,
        )
      }
      lines.push('')
      ...
      if (weightSnap.empty) {
        ...
        lines.push(
          `⚖️ Body Weight: ${formatWeight(w.weight as number, ctx.weightUnit)}`,
        )
      }
      ...
      if (calorieSnap.empty) {
        ...
        lines.push(
          `🔥 Calories: ${formatCalories(c.calories as number, ctx.energyUnit)}`,
        )
      }
      lines.push('')
      ...
      if (journalSnap.empty) {
        ...
        for (const doc of journalSnap.docs) {
          ...
          const supplements = j.supplements as
  ...
  server.tool(
    ...
    'Get a week-level overview with workout volume, frequency, weight trend, and calorie averages.',
    {
      week_offset: z
        ...
        .optional()
        .describe(
          '0 = current week, -1 = last week, -2 = two weeks ago, etc. Default: 0',
        ),
      timezone: z
        .string()
        .optional()
    ...
    async (args, extra) => {
      ...
      if (weights.length === 0) {
        ...
        const sign = delta >= 0 ? '+' : ''
        lines.push(
          `  Start: ${formatWeight(first, ctx.weightUnit)} → End: ${formatWeight(last, ctx.weightUnit)} (Δ ${sign}${delta.toFixed(1)} ${ctx.weightUnit})`,
        )
      ...
      if (calories.length === 0) {
        ...
        const total = calories.reduce(
          (sum, c) => sum + (c.calories as number),
          0,
        )
