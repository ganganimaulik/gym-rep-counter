...
import { formatDate, formatTime, formatDuration, formatWeight, formatShortDate } from '../formatters';
...
export function registerWorkoutTools(server: McpServer) {
  server.tool(
    ...
    {
      days_back: z.number().min(1).max(90).optional().describe('Number of days of history to fetch. Default: 7, max: 90'),
      exercise_name: z.string().optional().describe('Filter to a specific exercise name'),
    ...
    async (args, extra) => {
      ...
      const query = db.collection(`users/${user.uid}/history`)
        .where('date', '>=', start)
        .where('date', '<', end)
        .orderBy('date', 'desc');

      const snap = await query.get();
      let sets = snap.docs.map(d => ({ id: d.id, ...d.data() })) as Array<Record<string, unknown>>;

      // Filter by exercise name if provided
      if (args.exercise_name) {
        const filterName = args.exercise_name.toLowerCase();
        sets = sets.filter(s => (s.exerciseName as string).toLowerCase().includes(filterName));
      }

      if (sets.length === 0) {
        return { content: [{ type: 'text' as const, text: `📋 Workout History (Last ${daysBack} Days)\n\nNo workout sets found.` }] };
      }

      // Group by date
      const grouped: Record<string, Array<Record<string, unknown>>> = {};
      for (const s of sets) {
        const dateKey = getDateStringFromTimestamp(s.date as FirebaseFirestore.Timestamp, tz);
      ...
      for (const dateKey of sortedDates) {
        ...
        const exerciseGroups: Record<string, Array<Record<string, unknown>>> = {};
        ...
        for (const name of exerciseOrder) {
          ...
          for (let i = 0; i < exSets.length; i++) {
            ...
            lines.push(`    Set ${setNum}: ${reps} reps @ ${formatWeight(weight, ctx.weightUnit)} (${time})${restStr}`);
          }
        }
        lines.push('');
      }
  ...
  server.tool(
    ...
    'Log a completed workout set with exercise name, reps, and weight.',
    {
      ...
      weight: z.number().min(0).describe('Weight used (in your configured unit)'),
      set_number: z.number().int().min(1).optional().describe('Set number. Auto-incremented if omitted.'),
      workout_name: z.string().optional().describe('Name of the workout routine this set belongs to'),
      timezone: z.string().optional().describe('IANA timezone (e.g., "America/New_York"). Defaults to UTC.'),
    },
    async (args, extra) => {
      const user = getMcpUser(extra);
      ...
      if (args.workout_name) {
        ...
        if (userData?.workouts) {
          const workouts = userData.workouts as Array<{ id: string; name: string; exercises: Array<{ id: string; name: string }> }>;
          const workout = workouts.find(w => w.name.toLowerCase().includes(args.workout_name!.toLowerCase()));
          if (workout) {
            workoutId = workout.id;
            const exercise = workout.exercises.find(e => e.name.toLowerCase().includes(args.exercise_name.toLowerCase()));
            if (exercise) {
      ...
      if (!setNumber) {
        ...
        const todaySets = await db.collection(`users/${user.uid}/history`)
  ...
  server.tool(
    ...
    async (_args, extra) => {
      ...
      if (workouts.length === 0) {
        return { content: [{ type: 'text' as const, text: '📋 Your Workout Routines\n\nNo routines saved yet.' }] };
      }
  ...
  server.tool(
    ...
    async (args, extra) => {
      ...
      const allSets = snap.docs.map(d => d.data()) as Array<Record<string, unknown>>;

      if (allSets.length === 0) {
        return { content: [{ type: 'text' as const, text: '🏆 Personal Records\n\nNo workout data found in the last 90 days.' }] };
      }

      // Calculate PRs - max weight per exercise
      const prMap: Record<string, { name: string; maxWeight: number; repsAtMax: number; date: unknown }> = {};
      for (const s of allSets) {
        ...
        if (args.exercise_name && !exerciseName.toLowerCase().includes(args.exercise_name.toLowerCase())) {
          continue;
        }

        if (!prMap[exerciseId] || weight > prMap[exerciseId].maxWeight) {
          prMap[exerciseId] = { name: exerciseName, maxWeight: weight, repsAtMax: reps, date: s.date };
        }
      }
      ...
      for (let i = 0; i < sortedWeeks.length; i++) {
        ...
        if (qualifies) {
          if (i === 0) {
            ...
            const diffDays = Math.round((prevWeek.getTime() - thisWeek.getTime()) / (1000 * 60 * 60 * 24));
      ...
      if (prs.length === 0) {
        ...
        for (let i = 0; i < Math.min(prs.length, 10); i++) {
          ...
          lines.push(`${i + 1}. ${pr.name}: ${formatWeight(pr.maxWeight, ctx.weightUnit)} × ${pr.repsAtMax} reps (${dateStr})`);
        }
      }

      lines.push('');
      lines.push(`🔥 Streak: ${currentStreak} consecutive weeks (5+ workout days/week)`);
