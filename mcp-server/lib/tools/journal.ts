...
import { getDaysBackRange, getDayRange, getTodayString, getDateStringFromTimestamp } from '../date-utils';
...
export function registerJournalTools(server: McpServer) {
  server.tool(
    ...
    {
      days_back: z.number().min(1).max(90).optional().describe('Number of days of history. Default: 14'),
    ...
    async (args, extra) => {
      ...
      if (journalSnap.empty) {
        return { content: [{ type: 'text' as const, text: `📓 Journal Entries (Last ${daysBack} Days)\n\nNo journal entries found.` }] };
      }
      ...
      for (const doc of journalSnap.docs) {
        ...
        const badges: string[] = [];
        if (weightByDate[dateKey] !== undefined) {
          badges.push(`⚖️ ${formatWeight(weightByDate[dateKey], ctx.weightUnit)}`);
        }
        if (calorieByDate[dateKey] !== undefined) {
          badges.push(`🔥 ${formatCalories(calorieByDate[dateKey], ctx.energyUnit)}`);
        }

        const header = badges.length > 0
        ...
        const supplements = entry.supplements as Array<{ name: string; dosage?: string }> | undefined;
  ...
  server.tool(
    ...
    'Create a new journal entry with optional supplement tracking.',
    {
      ...
      supplements: z.array(z.object({
        ...
        dosage: z.string().optional().describe('Dosage (e.g., "5g", "1 capsule")'),
      })).optional().describe('Array of supplements taken'),
    ...
    async (args, extra) => {
      ...
      if (args.supplements && args.supplements.length > 0) {
        const suppStr = args.supplements.map(s => s.dosage ? `${s.name} (${s.dosage})` : s.name).join(', ');
        confirmation += `\n💊 Supplements: ${suppStr}`;
      }
