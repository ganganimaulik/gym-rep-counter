...
const pbxprojPath = path.join(
  __dirname,
  '../ios/repcounterapp.xcodeproj/project.pbxproj',
)
...
if (startMatch) {
  ...
  targetAttributesBlock = targetAttributesBlock.replace(
    /\s*DevelopmentTeam\s*=\s*[^;]+;/g,
    '',
  )
  targetAttributesBlock = targetAttributesBlock.replace(
    /\s*ProvisioningStyle\s*=\s*[^;]+;/g,
    '',
  )
  ...
  content =
    ...
    targetAttributesBlock +
    content.substring(endIndex)
}

fs.writeFileSync(pbxprojPath, content, 'utf8')
console.log(
  'Successfully configured Development Team 9F2FM5B6L2 (buildSettings & TargetAttributes) in project.pbxproj',
)
