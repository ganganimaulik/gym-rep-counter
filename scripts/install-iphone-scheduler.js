...
function getIdleTimeSeconds() {
  try {
    const output = execSync("ioreg -c IOHIDSystem | awk '/HIDIdleTime/ {print $NF/1000000000; exit}'").toString().trim();
...
function run() {
  ...
  const now = Date.now();
  const lastSuccess = state.lastSuccessTime ? parseInt(state.lastSuccessTime) : 0;
  const timeSinceLastSuccess = now - lastSuccess;

  console.log(`Time since last successful run: ${(timeSinceLastSuccess / (1000 * 60 * 60)).toFixed(2)} hours`);

  if (lastSuccess && timeSinceLastSuccess < INTERVAL_MS) {
    console.log("Check complete: Install ran successfully less than 2 days ago. Skipping.");
    process.exit(0);
  }

  // Check if laptop is idle
  const idleSeconds = getIdleTimeSeconds();
  console.log(`Current idle time: ${idleSeconds.toFixed(1)} seconds (threshold: ${IDLE_THRESHOLD_SECONDS} seconds)`);

  if (idleSeconds < IDLE_THRESHOLD_SECONDS) {
    console.log("Check complete: Laptop is NOT idle (user is active). Will retry on the next scheduled run.");
    process.exit(0);
  }
  ...
  if (result.status === 0) {
    console.log(`[${new Date().toISOString()}] npm run install-iphone completed successfully!`);
    ...
    console.error(`[${new Date().toISOString()}] Error: npm run install-iphone failed with exit code ${result.status}.`);
    process.exit(1);
  }
}
