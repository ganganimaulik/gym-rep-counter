...
export async function POST(req: Request) {
  ...
  if (!code) {
    return new Response(
      JSON.stringify({ error: 'invalid_request', error_description: 'code is required' }),
      { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders() } }
    );
  }
  ...
  if (codeChallenge && code_verifier) {
    if (!validatePKCE(code_verifier, codeChallenge, codeChallengeMethod)) {
      return new Response(
        JSON.stringify({ error: 'invalid_grant', error_description: 'PKCE validation failed' }),
        { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders() } }
      );
    }
  }
  ...
  return new Response(
    JSON.stringify({
      ...
      ...(googleTokens.refresh_token ? { refresh_token: googleTokens.refresh_token } : {}),
    }),
    {
      headers: {
