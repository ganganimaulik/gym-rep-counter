...
- **The journal's "day" rolls over at the configured wake-up hour, not midnight.** `supplementSchedule.getJournalDateKey(date, rolloverHour)` attributes any moment before the wake-up (sleep-end) hour to the previous calendar date — same hand-rolled format, so sorting still works. `JournalScreen` gets the hour via its `dayRolloverHour` prop, which `App.tsx` reads straight off `settings.statRemindersSleepEnd ?? 7` — the same user-set "Quiet End" the reminder quiet window uses. It is deliberately never derived: sleep auto-detection used to feed this and was removed (see below), because a day boundary that moves on its own silently re-dates journal entries. All journal/supplement "today" logic (due/taken checks, entry grouping, missed cutoffs) takes the optional `rolloverHour` trailing param, which defaults to `0` = calendar-day. `buildBedtimeReminderBody`/`buildSupplementReminderSignature` take it too and `notifications.ts` forwards its resolved `sleepEnd` — without that the evening reminder counts a 1 AM dose toward the wrong day and goes silent about a supplement the journal screen still lists as missing. Weight/calorie chips and TDEE stay calendar-keyed.
- **`getJournalDayDate` is idempotent, and everything downstream depends on it.** It anchors its result to `rolloverHour` local time, so the Date it returns is never itself "before the rollover" and cannot be shifted a second time. That is what makes two things safe: stepping back a day from it (the `every_other_day` branch) and rebuilding a Date from a journal-day key (`journalDayKeyToDate`, used by `JournalScreen`'s section header and missed-supplement footer). **Never rebuild a journal-day key with `new Date(y, m, d)`** — midnight is before any nonzero rollover, so every rollover-aware helper shifts it into the previous journal day and the section silently reports the wrong day's data. Anchoring to noon is not a substitute: `statRemindersSleepEnd` wraps mod 24, so a rollover hour above 12 is reachable, which is also why the journal date pickers normalize a picked date to `dayRolloverHour` rather than to 12:00.
- **The reminder quiet window is user-set only; there is no sleep auto-detection.** `settings.statRemindersSleepStart`/`SleepEnd` (defaults 23/7) are the single source, read by `notifications.setupReminders` and by the journal day boundary above. A `detectSleepWindow` heuristic that inferred the window from log timestamps was removed: real logging is far too sparse to locate a sleep window (in the author's own 60-day history the densest single hour appeared on only 25% of days, and recent days carried one event each), schedules shift faster than any lookback long enough to be dense, and the failure mode is an _inverted_ window that silences reminders all day and fires them all night. `SettingsModal` renders the manual +/- steppers whenever stat reminders are on. Don't reintroduce inference here.
